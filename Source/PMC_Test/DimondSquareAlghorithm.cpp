...
DimondSquareAlghorithm::DimondSquareAlghorithm(int size)
{
...
DimondSquareAlghorithm::~DimondSquareAlghorithm()
{
}

void DimondSquareAlghorithm::generateWorld()
{
	timeReset();
...
double DimondSquareAlghorithm::fRand2(double fMin, double fMax)
{
	double f = (double)rand() / RAND_MAX;
	return fMin + f * (fMax - fMin);
}

void DimondSquareAlghorithm::timeReset()
{
	srand(time(0)); //Seed the random system, important, otherwise the number the same
}

void DimondSquareAlghorithm::init_blocks_cords(DimondSquareAlghorithm algorithm)
{
	int block = 0, point = 0;
...
void DimondSquareAlghorithm::setScaleASmooth(int scale, int smooth)
{
...
std::map<int, std::vector<std::vector<double>>> DimondSquareAlghorithm::getBlocks()
{
